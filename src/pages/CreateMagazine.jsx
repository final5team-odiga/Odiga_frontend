...
export default function CreateMagazine() {
  ...
  const fetchImageList = async (magazineId) => {
    try {
      const res = await axiosInstance.get(`/storage/images/?magazine_id=${magazineId}`);
      const data = res.data;
  ...
  const fetchTextList = async (magazineId) => {
    try {
      const res = await axiosInstance.get(`/storage/texts/list/?magazine_id=${magazineId}`);
      const data = res.data;
      ...
      {mode === 'ai' ? (
        <div className="ai-generate-section">
          <div className="ai-folder-select">
            <label><FaFolderOpen style={{marginRight:'6px'}}/>매거진 폴더 선택</label>
          ...
          <div className="ai-image-list">
            <label><FaImage style={{marginRight:'6px'}}/>이미지 목록</label>
            <div className="ai-images">
              ...
              {aiImages.map((img, idx) => (
                <div key={idx} className="ai-image-item">
                  ...
                  <button onClick={() => handleDeleteImage(idx)} title="삭제">×</button>
                </div>
              ))}
              <button onClick={handleAddImage} className="ai-image-add-btn"><FaPlus/> 이미지 추가</button>
            </div>
          </div>
          <div className="ai-text-list">
            <label><FaFileAlt style={{marginRight:'6px'}}/>텍스트 목록</label>
            <ul>
              {aiTexts.length === 0 && <li className="ai-text-empty">텍스트 파일이 없습니다</li>}
              {aiTexts.map((txt, idx) => (
                <li key={idx}>{txt}</li>
              ))}
            </ul>
